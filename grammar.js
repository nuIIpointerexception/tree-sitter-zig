...
module.exports = grammar({
  ...
  precedences: ($) => [[$.container_field, $.type_expression]],
  ...
  rules: {
      ...
      choice(
        seq(
          ...
          repeat(
            prec(
              1,
              seq(",", choice($._variable_declaration_header, $.expression)),
            ),
          ),
          "=",
          $.expression,
    ...
    errdefer_statement: ($) =>
      seq("errdefer", optional($.payload), $._block_expr_statement),

    _block_expr_statement: ($) =>
      prec(
        1,
        choice(seq(optional($.block_label), $.block), $.expression_statement),
      ),
    ...
    if_statement: ($) => seq($._if_prefix, $._conditional_body),
    ...
    continue_expression: ($) =>
      prec.right(seq("continue", optional($.break_label))),
    ...
    break_expression: ($) =>
      prec.right(seq("break", optional($.break_label), optional($.expression))),
    ...
    anyframe_type: ($) => prec(1, seq("anyframe", "->", $.type_expression)),
    ...
    block_label: ($) =>
      prec(
        -1,
        seq(choice($.identifier, alias($.builtin_type, $.identifier)), ":"),
      ),
    break_label: ($) => seq(":", $.identifier),

    arguments: ($) => seq("(", optionalCommaSep($.expression), ")"),

    builtin_function: ($) => seq($.builtin_identifier, $.arguments),
      ...
      return token(
        choice(
          seq(
            ...
            ".",
            hexDigits,
            optional(seq(/[pP][+-]?/, decimalDigits)),
          ),
          seq(
            decimalDigits,
            ".",
            decimalDigits,
            optional(seq(/[eE][+-]?/, decimalDigits)),
          ),
    ...
    identifier: ($) =>
      choice($._identifier, $._reserved_identifier, seq("@", $.string)),
    _identifier: (_) => /[A-Za-z_][A-Za-z0-9_]*/,
    _reserved_identifier: (_) => choice("undefined", "null", "true", "false"),
    ...
    comment: ($) =>
      choice(
        ...
        prec(
          ...
          seq(
            choice("///", "//!"),
            repeat(choice($.doc_variable, token.immediate(/[^@\n]+/))),
          ),
        ),
        // commands
        prec(
    ...
    doc_variable: ($) => token.immediate(/@[A-Za-z_][A-Za-z0-9_]*/),

    command_tag: ($) =>
      choice(
        alias(seq("TODO", optional($.tag_name), ":"), $.todo_tag),
        alias(seq("FIXME", optional($.tag_name), ":"), $.fixme_tag),
        alias(seq("NOTE", optional($.tag_name), ":"), $.note_tag),
        alias(seq("HACK", optional($.tag_name), ":"), $.hack_tag),
        alias(seq("WARN", optional($.tag_name), ":"), $.warn_tag),
        alias(seq("PERF", optional($.tag_name), ":"), $.perf_tag),
        alias(seq("TEST", optional($.tag_name), ":"), $.test_tag),
      ),
